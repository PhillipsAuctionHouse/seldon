...
const TestInput = React.forwardRef(({defaultValue, ...props}: InputProps, ref: React.ForwardedRef<HTMLInputElement>) => {
...
describe('An Input', () => {
  ...
  it('will only render a warning message when input is not invalid', () => {
    const { rerender } = render(<Input {...reqProps} invalidText="invalid" warn warnText="Your input is in warning state"/>)
    expect(screen.queryByText(/Your input is in warning state/)).toBeInTheDocument();
  ...
  it('will not render a warning message or invalid message when input is disabled or readonly', () => {
    const { rerender } = render(<Input {...reqProps} disabled invalid invalidText="invalid" warn warnText="Your input is in warning state"/>)
    expect(screen.queryByText(/Your input is in warning state/)).not.toBeInTheDocument();
    expect(screen.queryByText(/invalid/)).not.toBeInTheDocument();
    rerender(<Input {...reqProps} readOnly invalid invalidText="invalid" warn warnText="Your input is in warning state"/>)
    expect(screen.queryByText(/Your input is in warning state/)).not.toBeInTheDocument();
    expect(screen.queryByText(/invalid/)).not.toBeInTheDocument();
  });
