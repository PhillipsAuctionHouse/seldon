...
export interface IconProps extends React.HTMLAttributes<HTMLDivElement> {
  ...
  /**
   * Color of the icon. Can be set to "currentColor" if you want to control the color through css. Otherwise, it only accepts valid seldon color tokens. Defaults to $black
   */
  color?: string;
  /**
...
const Icon = forwardRef<HTMLDivElement, IconProps>(
  ({ className, height, width, color, icon, isPictogram, ...props }: IconProps, ref) => {
    ...
    const componentProps = {
      color: color === 'currentColor' ? color : getScssVar(color ?? '', '$black'),
      ...(height ? { height } : {}),
      ...(width ? { width } : {}),
      ...commonProps,
    };
