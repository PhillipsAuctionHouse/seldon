...
const VolatileComponent = ({throwError = false}: WrappedProps) => {
...
describe('An ErrorBoundary', () => {
  ...
  it('will render a default fallback component if an error is thrown', () => {
    ...
    render(<ErrorBoundary logger={log as unknown as ErrorBoundaryProps['logger']}><VolatileComponent throwError /></ErrorBoundary>)
  ...
  it('will render a provided fallback component if an error is thrown', () => {
    const error = jest.spyOn(console, 'error').mockImplementation(() => {return});
    const log = jest.spyOn(console, 'log').mockImplementation(() => { return});
    render(<ErrorBoundary logger={log as unknown as ErrorBoundaryProps['logger']} fallback={<p>Something is not right!</p>}><VolatileComponent throwError /></ErrorBoundary>)
    expect(screen.queryByTestId('test-id')).not.toBeInTheDocument();
  ...
  it('will call the logger method that is passed in with the error details', () => {
    ...
    render(<ErrorBoundary logger={mockedLogger as unknown as ErrorBoundaryProps['logger']} fallback={<p>Something is not right!</p>}><VolatileComponent throwError /></ErrorBoundary>);
