...
describe('A DatePicker', () => {
  ...
  it('will update the input with a newly selected date', async () => {
    ...
    render(<DatePicker  {...reqProps} onChange={mockedOnChange} defaultValue={['2023-06-01T08:30', '2023-06-05T08:30']}/>)
    ...
    await waitFor(() => expect((screen.getByTestId('test-id') as HTMLInputElement).value).toEqual('2023-06-07 to 2023-06-08'));
  ...
  it('will call onChange callback if input is manually entered', async () => {
    const mockedOnChange = jest.fn();
    render(<DatePicker  {...reqProps} onChange={mockedOnChange} defaultValue={['2023-06-01T08:30', '2023-06-05T08:30']} allowInput/>)
    const input = screen.getByTestId('test-id')
    userEvent.click(input);
